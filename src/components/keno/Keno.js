...
class Keno extends React.Component {
  ...
  randomAll() {
    let number = [];
    let numbertmp = [];
    let length1 = this.randomLength();
    if (length1 === 0) {
      length1 = length1 + 1;
    }
    let length2 = this.randomLength();
    if (length2 === 0) {
      length2 = length2 + 1;
    }
    let length3 = this.randomLength();
    if (length3 === 0) {
      length3 = length3 + 1;
    }
  ...
  renderSystemKeno() {
    if (this.state.systemKeno === 1) {
      return (
        <div style={{ paddingTop: 10, fontFamily: "Helvetica" }}>
  ...
  renderAmountEO() {
    const amountEO = this.state.arrAmountEO.map((item, index, data) => {
      return (
        <div
          key={index}
          className={item.active ? styles.btnAmountSelect : styles.btnAmount}
          onClick={() => this.clickAmountEO(item.value)}
        >
          {item.text}
        </div>
      );
    ...
    return (
      <div className={styles.tableAmount}>
        <div className={styles.btnGroupAmount}>{amountEO}</div>
      </div>
    );
  }
  ...
  clickAmountEO = value => {
    let amountActive = 0;
    let amountOld = 0;
    const _arrAmountNew = this.state.arrAmountEO.map((item, index, data) => {
      let _item = {};
      if (item.active) amountOld = item.value;
      if (item.value == value) {
        _item = item;
        amountActive = item.value;
        _item.active = true;
      } else {
        _item = item;
        _item.active = false;
    ...
    let _totalAmount = this.state.totalAmountParity;
    if (this.state.isEvenNumber || this.state.isOddNumber) {
      _totalAmount = _totalAmount - amountOld;
      _totalAmount += this.state.systemKenoParity * amountActive;
    }

    this.setState({
      arrAmountEO: _arrAmountNew,
      totalAmountParity: _totalAmount
    });
  };
  ...
  clickAmountBS = value => {
    let amountActive = 0;
    let amountOld = 0;
    const _arrAmountNew = this.state.arrAmountBS.map((item, index, data) => {
      let _item = {};
      if (item.active) amountOld = item.value;
      if (item.value == value) {
        _item = item;
    ...
    let _totalAmount = this.state.totalAmountParity;
    if (this.state.isBigNumber || this.state.isSmallNumber) {
      _totalAmount = _totalAmount - amountOld;
      _totalAmount += this.state.systemKenoParity * amountActive;
    }

    this.setState({
      arrAmountBS: _arrAmountNew,
      totalAmountParity: _totalAmount
    });
  };

  renderAmountBS() {
    const amountBS = this.state.arrAmountBS.map((item, index, data) => {
      return (
        <div
          key={index}
          className={item.active ? styles.btnAmountSelect : styles.btnAmount}
          onClick={() => this.clickAmountBS(item.value)}
        >
          {item.text}
        </div>
      );
    ...
    return (
      <div className={styles.tableAmount}>
        <div className={styles.btnGroupAmount}>{amountBS}</div>
      </div>
    );
  }

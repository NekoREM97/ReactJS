...
import { Text, Carousel, WingBlank, Button, WhiteSpace, Flex, Picker, Toast, ActivityIndicator } from 'antd-mobile';
...
import { stringToNumberFormat, bag, getBagName, initBag, getPriceByBagPowers, getWeekdaysShort } from '../../utils/Helper';
import SelectNumberPower655 from './SelectNumberPower655';
import SelectSystemPower from './SelectSystemPower';
...
class Power655 extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            aNumber: ['', '', '', '', '', ''],
    ...
    componentWillReceiveProps(nextProps) {
        ...
        this.setState({ amtPower: localStorage.getItem("amountPower") })
        // if (nextProps.responseMain !== undefined) {
        //     if (nextProps.responseMain.Code === "00") {
        //         nextProps.responseMain.ListValue.map((item, index, data) => {
        //             if (item.Type === 1) {
        //                 this.setState({ amtPower: item.Amount })
        //             }
        //         })
        //     } else {
        //         Toast.info("Không tải được thông tin giải thưởng", 5);
        //         this.props.onBack();
        //     }
        // }
    }
    ...
    randomFast() {
        ...
        if (!this.state.aIsCheck) {
            ...
            const _totalAmount = this.state.totalAmount + (this.state.price * this.state.system.length);
            this.setState({ totalAmount: _totalAmount, fNumber: number.sort((a, b) => a - b), fIsCheck: true })
        }
    }
    randomAll() {
        let number = [];
    ...
    clearData = (type) => {
        switch (type) {
            case "A":
                this.setState({ aNumber: initBag(this.state.bagitem), aIsCheck: false })
                break;
            case "B":
                this.setState({ bNumber: initBag(this.state.bagitem), bIsCheck: false })
                break;
            case "C":
                this.setState({ cNumber: initBag(this.state.bagitem), cIsCheck: false })
                break;
            case "D":
                this.setState({ dNumber: initBag(this.state.bagitem), dIsCheck: false })
                break;
            case "E":
                this.setState({ eNumber: initBag(this.state.bagitem), eIsCheck: false })
                break;
            case "F":
                this.setState({ fNumber: initBag(this.state.bagitem), fIsCheck: false })
                break;
            default:
                break;
        }
    ...
    initfNumber = () => {
        const number = this.state.fNumber.map((item, index, data) => {
            return (
                <Text key={index} className={styles.text_number_circle} onClick={this.showModal("modal", "F")}>{item}</Text>
            );
        });
        return (
            <div className={styles.row_number}>
                <Text className={styles.text_header} style={{ float: "left", width: 40 }}>F</Text>
                <div style={{ width: "100%" }}>
                    {number}
                </div>
    ...
    buttonBeforeNumber = (type) => {
        return (
            ...
            //     <Button className={styles.button_number} style={{ backgroundColor: ColorButton, border: ColorButton }}
            //         onClick={() => this.randompower655(type)}
            //     >
            //         <CustomeIcon type={require("../../assets/sync.svg")}></CustomeIcon>
            //     </Button>
            // </div>
    ...
    buttonAfterNumber = (type) => {
        return (
            // <div style={{ float: "right" }}>
            //     <Button className={styles.button_number} style={{ backgroundColor: "#E32E35", border: "#E32E35" }}
            //         onClick={() => this.clearData(type)}
            //     >
            //         <CustomeIcon type={require("../../assets/trash.svg")}></CustomeIcon>
            //     </Button>
            // </div>
            <div className={random}>
                <Button type="warning" size="small" style={btnclearmax3d}
                    onClick={() => this.clearData(type)}
                ><CustomeTrashIcon type={require("../../assets/trash.svg")}></CustomeTrashIcon></Button>
            </div>
        );
    }
    ...
    isCheckNumber = (type) => {
        ...
        switch (type) {
            ...
            case "E":
                ...
                break;
            case "F":
                showType = this.state.fIsCheck === true ? this.buttonAfterNumber(type) : this.buttonBeforeNumber(type);
                break;
            default:
    ...
    onAcceptSystemModal = (key, data) => () => {
        if (data.length !== this.state.system.length) {
            let _totalAmount = this.state.totalAmount / this.state.system.length * data.length;
            this.setState({ totalAmount: _totalAmount })
        }
        this.setState({
            system: data,
    ...
    onChangeBag = (value) => {
        this.setState({
            ...
            price: getPriceByBagPowers(value),
            aIsCheck: false,
            bIsCheck: false,
            cIsCheck: false,
            dIsCheck: false,
    ...
    checkPaymega645() {
        if ((this.state.aIsCheck ||
            ...
            this.state.dIsCheck ||
            this.state.eIsCheck ||
            this.state.fIsCheck) && this.state.system.length > 0) {
    ...
    RandomAllButton = () => {
        ...
        else if (this.state.aIsCheck === true || this.state.bIsCheck === true || this.state.cIsCheck === true || this.state.dIsCheck === true || this.state.eIsCheck === true || this.state.fIsCheck === true) {
    ...
    render() {
        return (<div className={styles2.content}>
            ...
            <div className={styles2.div_button} style={{ marginLeft: 6, marginRight: 6 }}>
                <div style={{ display: 'flex', padding: '4px 0' }}>
                    <div>
                        <img className={styles2.div_img} src={require("../../assets/power.png")} alt="" />
                    ...
                    <div className={styles2.div_group_text}>
                        <div className={styles2.div_text_amt}>{this.state.amtPower.toString().substring(0, this.state.amtPower.toString().length - 9)}
            ...
            <WingBlank style={{ marginLeft: 6, marginRight: 6 }}>
                        ...
                        <Flex>
                            <div style={{ width: "50%" }}>
                                <Picker
                                    ...
                                    <Button className={styles.button_drop_down} style={{ width: '100%' }}>
                                        <Text> {getBagName(this.state.bagitem)}</Text>
                                        <CustomeIcon className={styles2.drop} type={require("../../assets/caret_down.svg")}></CustomeIcon>
                                    </Button>
                                </Picker>
                            </div>
                            <Button onClick={this.showSystemModal("modalSystem")} className={styles.button_drop_down} style={{ marginLeft: 4 }}>
                                <Text>{this.state.system.length === 1 ? this.state.system[0].text : "..."}</Text>
                                <CustomeIcon className={styles2.drop} type={require("../../assets/caret_down.svg")}></CustomeIcon>
                            </Button>
                        </Flex>
                    </div>
                ...
                <Button style={{ background: ColorButton, color: "#FFFFFF", marginLeft: 8, marginRight: 8, marginBottom: 8, height: "40px", lineHeight: "40px" }} onClick={() => this.randomFast()}>
                ...
                <div className={styles2.primary_btn} style={{ marginLeft: 8, marginRight: 8 }} onClick={() => { this.checkPaymega645() }}>
                    <Text>Đặt vé</Text>
                </div>
            </WingBlank>
        </div>)

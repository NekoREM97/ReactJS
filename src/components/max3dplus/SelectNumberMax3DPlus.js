...
class SelectNumberMax3DPlus extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            data_colum5: [false, false, false, false, false, false, false, false, false, false],
            data_colum6: [false, false, false, false, false, false, false, false, false, false],
        }
    }
    ...
    renderContent4 = () => {
        const datanew = this.state.data_colum4.map((item, index, data) => {
            if (index === data.length - 1) {
                return (
                    <Text key={index} className={item ? styles.select_modal_number_circle_last : styles.modal_number_circle_last} onClick={() => this.renderNumberColumn4(index)}>{index}</Text>
                );
            }
            else {
                return (
                    <Text key={index} className={item ? styles.select_modal_number_circle : styles.modal_number_circle} onClick={() => this.renderNumberColumn4(index)}>{index}</Text>);
            }
        });
        return (<WingBlank style={{
            margin: 0, width: "28%",
    ...
    renderContent5 = () => {
        const datanew = this.state.data_colum5.map((item, index, data) => {
            if (index === data.length - 1) {
                return (
                    <Text key={index} className={item ? styles.select_modal_number_circle_last : styles.modal_number_circle_last} onClick={() => this.renderNumberColumn5(index)}>{index}</Text>
                );
            }
            else {
                return (
                    <Text key={index} className={item ? styles.select_modal_number_circle : styles.modal_number_circle} onClick={() => this.renderNumberColumn5(index)}>{index}</Text>);
            }
        });
        return (<WingBlank style={{
            margin: 0, width: "28%",
    ...
    renderContent6 = () => {
        const datanew = this.state.data_colum6.map((item, index, data) => {
            if (index === data.length - 1) {
                return (
                    <Text key={index} className={item ? styles.select_modal_number_circle_last : styles.modal_number_circle_last} onClick={() => this.renderNumberColumn6(index)}>{index}</Text>
                );
            }
            else {
                return (
                    <Text key={index} className={item ? styles.select_modal_number_circle : styles.modal_number_circle} onClick={() => this.renderNumberColumn6(index)}>{index}</Text>);
            }
        });
        return (<WingBlank style={{
            margin: 0, width: "28%",
    ...
    accept = () => {
        ...
        if (value1 !== -1 && value2 !== -1 && value3 !== -1 && value4 !== -1 && value5 !== -1 && value6 !== -1) {
            return this.props.onAccept('modal', [value1, value2, value3,value4, value5, value6], this.props.modalType)();
        }
        else{
            new Promise(() => {

    ...
    if (this.props.type !== prevProps.type) {
      ...
      if (dataNumber !== undefined && dataNumber[0] !== "") {
        dataNumber.map((item, index, data) => {
           if (index === 1) {
            this.updateColumn(item.substr(0, 1), 1);
            this.updateColumn(item.substr(1, 1), 2);
            this.updateColumn(item.substr(2, 1), 3);
            this.updateColumn(item.substr(3, 1), 4);
            this.updateColumn(item.substr(4, 1), 5);
          }
        });
      } else {
        this.setState({
